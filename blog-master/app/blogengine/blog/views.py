...
def download_file_txt(request, slug):
    ...
    для пользователя это выглядит как всплывающее окно с продложенем сохранить файл
    """
    your_string = '-'.join([str(i) for i in Post.objects.filter(slug=slug)]).encode("utf-8")
    print(your_string)
    f = open('blog/files/blog.txt', 'wb')
    f.write(your_string)
...
def comment_delete_profile(request, id, author_post):
    """
    ...
    """
    print("dekete_coment")
    Commets_profile.objects.filter(id=id).delete()
    return redirect('profile', author_post)